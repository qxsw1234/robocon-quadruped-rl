"""Unitree A1 velocity environment configurations (custom robot for mjlab).

川山甲战队考核项目 3.2 拓展题：把自研流程演示为"在 mjlab 中接入 12 自由度 A1"。
结构完全模仿 mjlab 自带 unitree_go1/env_cfgs.py，仅替换机器人相关配置。
"""
...
def unitree_a1_rough_env_cfg(
  ...
  # Set raycast sensor frame to A1 trunk.
  ...
  geom_names = tuple(f"{name}_foot" for name in foot_names)
  ...
  trunk_collision_names = tuple(
    f"trunk_collision{i}" for i in range(1, 9)
  )
  ...
  joint_pos_action.scale = A1_ACTION_SCALE
